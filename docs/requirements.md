...
- `thread_tokens.access_token_encrypted` にKMS暗号化済みアクセストークンを保存する
...
- 休止時はStripeサブスクリプションを停止し、再開後に利用する場合は再登録が必要になる
...
- Stripe Checkoutのsuccess_url / cancel_urlは環境ごとのフロントエンドURLから生成し、dev確認時に本番URLへ戻さない
- 有効なサブスクリプションまたは14日間無料トライアル期間内のユーザーのみ予約投稿できる
- 14日間無料トライアル終了後、サブスクリプションが`active`でないユーザーは予約作成、予約編集、予約削除、投稿実行、分析取得を利用できない
- 無料トライアル中でもすぐにStripe Checkoutへ進める「今すぐ登録」ボタンを表示する
- 無料トライアルは初回登録前のお試し期間として扱い、一度Stripeサブスクリプション登録が完了したThreadsアカウントは無料トライアルに戻れない
- Stripeサブスクリプション登録が完了した時点で、アプリ側の無料トライアル権限を終了する
- 休止・退会後に再開または再ログインして利用する場合は、Stripeサブスクリプションへの再登録が必要になる
- ユーザー向け表示価格は税込390円/月とする
- 無料トライアル中は残り日数と残り時間を表示する
- 無料トライアルの開始日時と終了日時を設定画面、および期限切れ時の案内に表示する
- 無料トライアルの終了日時が未来の場合は、期限切れバナーを表示しない
- 無料トライアルは初回Threadsログイン時、または初回トライアル資格レコード作成時に開始する
- 無料トライアルは同一Threadsアカウントにつき1回のみ利用可能とする
- 既存ユーザーに `trial_end` が未保存の場合は、`trial_started_at`、`trial_eligibility.first_trial_started_at`、`created_at` の順で無料トライアル開始日時を補完し、その14日後を無料トライアル終了日時とする
...
- `trial_eligibility.trial_used` がtrueのThreadsアカウントでは、Stripe Checkout作成時に新しい `trial_period_days` を付与しない
- 解約だけの導線は設けず、ユーザー操作としては退会を基本導線にする
- 退会時はStripeサブスクリプションを終了し、削除対象データを削除する
- MVPでは休止状態を扱い、休止中はStripeサブスクリプション、予約作成、予約編集、予約削除、投稿実行、分析取得を停止する
...
- 過去日時への予約は不可
- 現在時刻から5分以内の日時には予約不可
- 予約可能期間は現在時刻から30日以内とし、30日を超える日時への予約は不可
- 投稿本文の空文字は不可
...
- 予約済み投稿は投稿前であれば編集できる
- 予約済み投稿は投稿前であれば削除できる
- 編集時も1日3件制限、5分以内禁止、30日以内制限、サブスクリプション状態、休止状態、Threads再連携要否を再チェックする
...
- メイン画面の日付は固定文言ではなく、ユーザーのタイムゾーンに合わせた現在日付を表示する
- 表示言語の変更は設定画面に集約し、メイン画面のtopbarには表示言語セレクトを置かない
- 設定画面の課金状態は「トライアル中」「登録済み」「登録が必要」などの状態名を表示し、残り時間はトライアル期間表示に集約する
- 設定画面のThreads再連携ボタンは、Threads連携が再連携必要な時だけThreads連携行内に表示する
- 設定画面の休止・退会ボタンは課金中のユーザーにのみ表示し、トライアル中は表示しない
- 設定画面に休止と退会の意味を表示する。休止はアカウントを残したサブスクリプション停止と利用停止、退会はサブスクリプション終了と対象データ削除として説明する
- 設定画面の法務文書はMarkdown直リンクではなくアプリ内表示とし、文字化けを避ける
- 主要UI文言は設定画面の表示言語に合わせて、日本語、英語、中国語、Filipino、ベトナム語で表示する
- ログイン画面にも表示言語切替を置き、ログイン前でも日本語、英語、中国語、Filipino、ベトナム語で案内文言を切り替えられるようにする
- ログイン画面の表示言語切替は見つけやすさのため国旗と表示言語名を併記する
- ログイン画面から特定商取引法に基づく表記、利用規約、プライバシーポリシーを確認できる
- 日本向け法務文書の本文は日本語原文を維持し、法務文書への導線ラベルは表示言語に合わせる
...
- 特定商取引法に基づく表記の電話番号は、請求があった場合に遅滞なく開示する表記とし、ログイン前画面やアプリ内法務表示には電話番号そのものを表示しない
...
- `POST /auth/logout`
- `GET /me`
- `GET /auth/threads/start?reauth=1`
...
`trial_used` はアプリ側の14日無料トライアルをすでに開始済みであることを示す。退会後に同じThreadsアカウントで再登録した場合も、同じ `trial_key_hash` を参照して再トライアルを付与しない。
...
`failure_detail` は内部調査用の分類情報とし、外部APIレスポンス全文、投稿本文、アクセストークン、Cookie、Authorizationヘッダーは保存しない。
...
上記は投稿・分析バックグラウンド処理中心の概算であり、管理画面の表示回数、CloudWatch Logs、API Gateway、Lambda実行時間、KMS復号、DynamoDB item size、GSI数により増減する。390円税込の低単価プランのため、本番運用でユーザー数が増える前にDynamoDB Scanを棚卸しし、GSIで必要な範囲だけQueryする。

dev/MVP時点では、管理者画面、退会時の関連データ削除、予約件数チェック、分析データ取得などにScanが残る可能性を許容する。ただし本番公開前チェックでは以下を確認する。

- 一般ユーザー操作で高頻度に走るScanを洗い出す
- 予約一覧、分析一覧、日次予約件数、投稿削除、退会削除は `threads_user_id`、`app_user_id`、`status`、`scheduled_at`、`post_id` を使ったQueryへ移行できるか確認する
- 管理者画面の全体集計Scanは初期運用では許容し、データ量が増える前に集計テーブルまたはGSIへ移行する
- Token refreshの全件Scanは初期運用では許容し、ユーザー数増加前に `expires_at` ベースのGSIまたは分割実行へ移行する
- Scanを残す場合は、対象テーブル、想定件数、実行頻度、移行条件を運用メモに残す
