...
メイン画面の日付が `2026年5月1日 金曜日` に固定されていた問題を修正

ユーザーのタイムゾーンに合わせて現在日付を表示するように変更

無料トライアル終了日時が未来の場合に、期限切れバナーを表示しないようにフロントエンド側の表示判定を補強

DynamoDB上で `subscription_status=incomplete` かつ `trial_end` が未来の状態を確認し、サブスクリプション状態が `active` 以外でも `trial_end` が未来なら画面上はトライアル中として扱うように追加補強

topbarの表示言語セレクトを削除し、表示言語の変更は設定画面に集約

設定画面の課金状態は残り時間ではなく「トライアル中」などの状態名を表示するように変更

Threads連携が有効な場合は独立した再連携ボタンを表示せず、再連携が必要な場合だけThreads連携行内に再連携ボタンを表示するように変更

設定画面の休止・退会ボタンは課金中のユーザーにのみ表示し、トライアル中は非表示に変更

設定画面に休止と退会の意味を説明する補足文を追加

設定画面の法務文書リンクをMarkdown直リンクからアプリ内モーダル表示へ変更し、文字化けしないように修正

主要UI文言を設定画面の表示言語に合わせて、日本語、英語、中国語、Filipino、ベトナム語で表示する辞書を追加

ナビゲーション、課金表示、予約作成、予約一覧、分析、設定、確認モーダルの表示文言を多言語対応

ログイン画面を2カラム構成へ刷新し、サービス概要、利用開始ボタン、14日間無料トライアル案内を整理

ログイン画面にも国旗付きの表示言語切替を追加し、ログイン前から日本語、英語、中国語、Filipino、ベトナム語の案内文言へ切り替え可能に変更

ログイン前に選んだ表示言語をブラウザ内へ保存し、次回表示時の初期表示へ反映

### ドキュメント

要件定義に、メイン画面の日付はユーザーのタイムゾーンに合わせた現在日付を表示することを追記

要件定義に、無料トライアル終了日時が未来の場合は期限切れバナーを表示しないことを追記

要件定義に、表示言語の変更は設定画面に集約することを追記

要件定義に、設定画面の課金状態は状態名を表示し、残り時間はトライアル期間表示に集約することを追記

要件定義に、Threads再連携ボタンは再連携が必要な場合だけThreads連携行内に表示することを追記

要件定義に、休止・退会ボタンは課金中のみ表示し、それぞれの意味を設定画面で説明することを追記

要件定義に、設定画面の法務文書はアプリ内表示にすることを追記

要件定義に、主要UI文言を日本語、英語、中国語、Filipino、ベトナム語で表示することを追記

要件定義に、ログイン画面の表示言語切替と国旗付き表示を追記
...
モバイル端末でログイン後にログイン画面へ戻る問題に対応

フロントエンドのOAuth戻り先に `VITE_APP_URL` を優先して使うように変更

OAuth戻り先の許可リストに `https://dev.s4s.aokigk.com` と環境変数 `APP_URL` を反映できるように変更

退会APIが分析データを削除できるよう、API Lambdaへ `PostAnalyticsTable` のDynamoDB権限を追加

休止・退会の説明文を、Threads本体の投稿は削除されず本アプリ上の記録や連携情報が対象であることが分かる内容へ変更

退会確認ダイアログにも、Threads本体の投稿は削除されない旨を追加

### 追加

退会時のStripeサブスクリプションキャンセルに失敗した場合、退会処理は続行しつつ管理者確認用の状態をDBへ残すように変更

`subscriptions` と `users` に `requires_admin_review`、`admin_review_reason`、`stripe_cancel_failed_at`、`stripe_cancel_error` を保存

Stripeキャンセル失敗時の `subscriptions.status` を `cancel_pending_admin_review` として保存

開発者Threads ID `26271207252581685` のユーザーだけに開発者メニューを表示するように追加

`/me` レスポンスに `is_developer` を追加

開発者専用API `GET /developer/dashboard` を追加

開発者画面に、管理者確認が必要な退会/Stripeキャンセル失敗ユーザー一覧を表示

開発者画面に、有効ユーザー数、トライアル人数、サブスク人数、CVR、要確認件数、サブスク記録数を表示

CVRは `サブスク人数 / (トライアル人数 + サブスク人数)` で算出

### 検証

`npm run build` 成功

`PYTHONPYCACHEPREFIX=/private/tmp/codex-pycache python3 -m py_compile backend/app/handlers/api.py` 成功

## 2026-05-07 JST（続き）

### 追加

投稿成功後に分析取得用のEventBridge Schedulerを作成する処理を追加

分析取得は投稿後1時間、24時間、72時間の3回実行するように追加

`analytics_sync` LambdaでThreads投稿Insightsを取得し、`post_analytics` に保存する処理を実装

取得対象メトリクスは `views`、`likes`、`replies`、`reposts`、`quotes`、`shares` とし、`engagement_total` も保存

分析同期時に、投稿済み状態、Threads media ID、ユーザー状態、サブスクリプション状態、Threadsトークン状態を確認するように追加

`GET /analytics` で保存済み分析データからsummaryと投稿別ランキングを返すように追加

`GET /scheduled-posts` の投稿一覧レスポンスへ、保存済みの最新分析データを含めるように変更

分析画面に分析済み投稿数、最新取得ステージ、取得日時、データ未取得時の案内文を追加

### 変更

メイン画面のMeta App Review向け通知帯を削除

モバイルで開発者、予約一覧、分析メニューを開いたときにサイドバーとナビゲーションが横に広がらないように調整

モバイル時の開いたナビゲーションを `auto-fit` グリッドへ変更し、メニュー数が増えても幅内に収まるように変更

携帯表示でtop化したサイドバーが画面高さいっぱいに引き伸ばされる問題を修正

携帯表示時のアプリ全体gridを `auto 1fr` 行に固定し、topナビゲーションを中身の高さで表示するように変更

### インフラ

`PostExecutorFunction` に分析同期Lambda ARN、Scheduler Group、Scheduler Invoke Role ARNを渡す環境変数を追加

`PostExecutorFunction` にEventBridge Scheduler作成権限とInvoke RoleのPassRole権限を追加

`AnalyticsSyncFunction` にUsersTable読み取り権限とKMS復号権限を追加

Scheduler Invoke Roleから `PostExecutorFunction` と `AnalyticsSyncFunction` を呼び出せるように権限を調整

### 検証

`npm run build` 成功

`PYTHONPYCACHEPREFIX=/private/tmp/codex-pycache python3 -m py_compile backend/app/handlers/api.py backend/app/handlers/post_executor.py backend/app/handlers/analytics_sync.py` 成功
...
開発者画面の管理者確認一覧に「確認済みにする」ボタンを追加

開発者専用API `POST /developer/admin-review/resolve` を追加

管理者確認を解消した場合、`subscriptions` と `users` に `requires_admin_review=false`、`admin_review_resolved_at`、`admin_review_resolved_by` を保存するように追加

カレンダーの日付横スクロールで、表示月に今日が含まれる場合は今日の日付が中央付近に来るように初期スクロールを追加

### 変更

開発者判定を `admins` テーブル優先に変更

環境ごとの初期管理者として `BOOTSTRAP_ADMIN_THREADS_USER_ID` を追加し、`admins` テーブル未登録時のブートストラップ用フォールバックとして使用

`PublishPostFunction` は未使用のためSAMテンプレートから削除

未使用の `backend/app/handlers/publish_post.py` を削除

休止時に未投稿予約のEventBridge Schedulerを削除し、予約ステータスを `canceled` に変更するように変更

休止・再開後に予約一覧を再取得し、画面上の予約状態を最新化するように変更

設定画面の休止・退会説明に、未投稿予約のキャンセルと休止後の予約非復元を明記

休止実行前に、未投稿予約がキャンセルされ再開しても自動復活しないことを確認ダイアログで表示
...
休止時にStripeサブスクリプションをキャンセルし、`subscriptions.status` と `users.subscription_status` を `canceled` に更新するように変更

休止時のStripeキャンセルに失敗した場合、退会時と同じく管理者確認フラグと失敗理由を保存するように変更

休止実行後にフロントエンドの課金状態とサブスクリプション権限を即時更新するように変更

設定画面の休止説明と確認ダイアログに、Stripeサブスクリプション停止と再開後の再登録が必要なことを明記
...
Threads OAuthの `redirect_uri` をdev固定値から `THREADS_REDIRECT_URI` 環境変数へ変更し、SAMテンプレートでAPIカスタムドメインから生成するように変更

APIのOPTIONS応答で、環境ごとの `APP_URL` または許可済みoriginを `Access-Control-Allow-Origin` に返すように変更

Stripe Billing PortalのモックURLを廃止し、Stripe Billing Portal sessionを作成して支払い方法変更画面へ遷移できるように変更

設定画面の課金中ユーザー向け操作に「支払い方法を変更」ボタンを追加
...
ログイン画面から特定商取引法に基づく表記、利用規約、プライバシーポリシーを確認できるリンクを追加

DynamoDB Scanの本番リスクについて、dev/MVPで許容する範囲と本番運用前にQuery/GSI化を検討する対象を要件定義へ追記

### 変更

特定商取引法に基づく表記の電話番号を、請求があった場合に遅滞なく開示する表記へ変更

無料トライアル中に有料プランへ登録した場合、その時点で無料トライアルが終了し、退会・休止後も残日数が復活しない旨を法務文書へ追記
...
無料トライアルを「初回登録前のお試し期間」と定義し、Stripeサブスクリプション登録完了時点でアプリ側の無料トライアル権限を終了するように変更

休止・退会時にもアプリ側の無料トライアル権限を終了し、再開または再ログイン後の利用にはStripe再登録が必要になるように変更

フロントエンドで `trial_end` が未来という理由だけで `subscription_status` を `trialing` 表示へ補正する処理を削除し、バックエンドの課金状態と権限判定をそのまま表示に使うように変更
...
モバイル表示時に予約作成フォームの日時入力がフォーム幅からはみ出ないよう、フォームコントロールの最小幅と最大幅を調整

iPhone Safariで入力欄フォーカス後に画面が自動ズームされるのを避けるため、フォームコントロールの文字サイズを16pxに変更
...
予約作成画面の残り予約枠表示を `残 X/3 枠` に変更

設定画面の休止・退会説明文を常時表示からヘルプボタン付きモーダル表示へ変更
